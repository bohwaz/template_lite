...
function compile_compile_if($arguments, $elseif, $while, &$object)
{
	...
	preg_match_all('/(?>(' . $object->_const_regexp . '|\/?' . $object->_var_regexp . '|\/?' . $object->_svar_regexp . '|\/?' . $object->_func_regexp . ')(?:' . $object->_mod_regexp . '*)?|\-?0[xX][0-9a-fA-F]+|\-?\d+(?:\.\d+)?|\.\d+|!==|===|==|!=|<>|<<|>>|<=|>=|\&\&|\|\||\(|\)|,|\!|\^|=|\&|\~|<|>|\%|\+|\-|\/|\*|\@|\b\w+\b|\S+)/x', $arguments, $_match);
	$_args = $_match[0];
	...
	for ($i = 0, $for_max = $count_args; $i < $for_max; $i++)
	{
		...
		switch (strtolower($_arg))
		{
			...
			case 'is':
				...
				break;
			default:
				if (defined($_arg) && preg_match('/^'.$object->_const_regexp.'$/', $_arg))
				{
					$_arg = "constant('".$_arg."')";
					break;
				}
